...
package es.juntadeandalucia.nube.files;
...
import es.juntadeandalucia.nube.lib.common.network.WebdavUtils;
...
import es.juntadeandalucia.nube.ui.activity.ShareActivity;
import es.juntadeandalucia.nube.ui.dialog.ShareLinkToDialog;
...
public class FileOperationsHelper {
                ...
                try {
                    mFileActivity.startActivity(
                            Intent.createChooser(
                                    openFileWithIntent, mFileActivity.getString(es.juntadeandalucia.nube.R.string.actionbar_open_with)
                            )
                    );
    ...
    private void showNoAppForFileTypeToast(Context context) {
        Toast.makeText(context,
                es.juntadeandalucia.nube.R.string.file_list_no_app_for_file_type, Toast.LENGTH_SHORT)
                .show();
    }
    ...
    public void shareFileViaLink(OCFile file, String password) {
        if (isSharedSupported()) {
            if (file != null) {
                mFileActivity.showLoadingDialog(
                        mFileActivity.getApplicationContext().
                                getString(es.juntadeandalucia.nube.R.string.wait_a_moment)
                );
            ...
            Toast t = Toast.makeText(
                    mFileActivity, mFileActivity.getString(es.juntadeandalucia.nube.R.string.share_link_no_support_share_api),
                    Toast.LENGTH_LONG
            );
            t.show();
        }
    ...
    private void queueShareIntent(Intent shareIntent){
        if (isSharedSupported()) {
            ...
            Toast t = Toast.makeText(mFileActivity,
                    mFileActivity.getString(es.juntadeandalucia.nube.R.string.share_link_no_support_share_api),
                    Toast.LENGTH_LONG);
            t.show();
    ...
    public void checkCurrentCredentials(Account account) {
        ...
        mFileActivity.showLoadingDialog(
            mFileActivity.getApplicationContext().getString(es.juntadeandalucia.nube.R.string.wait_checking_credentials)
        );
    }
}
