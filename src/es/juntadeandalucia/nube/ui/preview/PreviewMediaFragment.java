...
import es.juntadeandalucia.nube.files.FileMenuFilter;
import es.juntadeandalucia.nube.lib.common.utils.Log_OC;
import es.juntadeandalucia.nube.media.MediaControlView;
import es.juntadeandalucia.nube.media.MediaService;
import es.juntadeandalucia.nube.media.MediaServiceBinder;
...
import es.juntadeandalucia.nube.ui.dialog.RemoveFileDialogFragment;
...
public class PreviewMediaFragment extends FileFragment implements
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mView = inflater.inflate(es.juntadeandalucia.nube.R.layout.file_preview, container, false);

        mImagePreview = (ImageView) mView.findViewById(es.juntadeandalucia.nube.R.id.image_preview);
        mVideoPreview = (VideoView) mView.findViewById(es.juntadeandalucia.nube.R.id.video_preview);
        mVideoPreview.setOnTouchListener(this);

        mMediaController = (MediaControlView) mView.findViewById(es.juntadeandalucia.nube.R.id.media_controller);
        ...
        if (file.isAudio()) {
            try {
                ...
                if (data != null) {
                    ...
                    mImagePreview.setImageResource(es.juntadeandalucia.nube.R.drawable.logo);
                }
            } catch (Throwable t) {
                mImagePreview.setImageResource(es.juntadeandalucia.nube.R.drawable.logo);
            }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        inflater.inflate(es.juntadeandalucia.nube.R.menu.file_actions_menu, menu);
    }
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        MenuItem item = menu.findItem(es.juntadeandalucia.nube.R.id.action_rename_file);
        if (item != null) {
        ...
        // additional restriction for this fragment
        item = menu.findItem(es.juntadeandalucia.nube.R.id.action_move);
        if (item != null) {
            item.setVisible(false);
            item.setEnabled(false);
        }
        ...
        // additional restriction for this fragment
        item = menu.findItem(es.juntadeandalucia.nube.R.id.action_copy);
        if (item != null) {
            item.setVisible(false);
            item.setEnabled(false);
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case es.juntadeandalucia.nube.R.id.action_share_file: {
                seeShareFile();
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_open_file_with: {
                openFile();
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_remove_file: {
                RemoveFileDialogFragment dialog = RemoveFileDialogFragment.newInstance(getFile());
                dialog.show(getFragmentManager(), ConfirmationDialogFragment.FTAG_CONFIRMATION);
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_see_details: {
                seeDetails();
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_send_file: {
                sendFile();
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_sync_file: {
                mContainerActivity.getFileOperationsHelper().syncFile(getFile());
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_favorite_file:{
                mContainerActivity.getFileOperationsHelper().toggleFavorite(getFile(), true);
                return true;
            }
            case es.juntadeandalucia.nube.R.id.action_unfavorite_file:{
