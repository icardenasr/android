...
public class ErrorsWhileCopyingHandlerActivity  extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /// load generic layout
        setContentView(es.juntadeandalucia.nube.R.layout.generic_explanation);
        
        /// customize text message
        TextView textView = (TextView) findViewById(es.juntadeandalucia.nube.R.id.message);
        String appName = getString(es.juntadeandalucia.nube.R.string.app_name);
        String message = String.format(getString(es.juntadeandalucia.nube.R.string.sync_foreign_files_forgotten_explanation),
                appName, appName, appName, appName, mAccount.name);
        textView.setText(message);
        textView.setMovementMethod(new ScrollingMovementMethod());
        
        /// load the list of local and remote files that failed
        ListView listView = (ListView) findViewById(es.juntadeandalucia.nube.R.id.list);
        ...
        /// customize buttons
        Button cancelBtn = (Button) findViewById(es.juntadeandalucia.nube.R.id.cancel);
        Button okBtn = (Button) findViewById(es.juntadeandalucia.nube.R.id.ok);
        
        okBtn.setText(es.juntadeandalucia.nube.R.string.foreign_files_move);
        ...
        public View getView (int position, View convertView, ViewGroup parent) {
            ...
            if (view != null)  {
                ...
                if (localPath != null) {
                    ...
                    if (text1 != null) {
                        text1.setText(String.format(getString(es.juntadeandalucia.nube.R.string.foreign_files_local_text), localPath));
                    }
                }
                if (mRemotePaths != null && mRemotePaths.size() > 0 && position >= 0 &&
                    ...
                    if (text2 != null && remotePath != null) {
                        text2.setText(String.format(getString(es.juntadeandalucia.nube.R.string.foreign_files_remote_text), remotePath));
                    }
                }
            }
            return view;
    ...
    private class MoveFilesTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute () {
            ...
            mCurrentDialog = IndeterminateProgressDialog.newInstance(es.juntadeandalucia.nube.R.string.wait_a_moment, false);
            mCurrentDialog.show(getSupportFragmentManager(), WAIT_DIALOG_TAG);
            findViewById(es.juntadeandalucia.nube.R.id.ok).setEnabled(false);
        }
        ...
        protected void onPostExecute(Boolean result) {
            ...
            mCurrentDialog.dismiss();
            mCurrentDialog = null;
            findViewById(es.juntadeandalucia.nube.R.id.ok).setEnabled(true);
            
            if (result) {
                // nothing else to do in this activity
                Toast t = Toast.makeText(ErrorsWhileCopyingHandlerActivity.this,
                        getString(es.juntadeandalucia.nube.R.string.foreign_files_success), Toast.LENGTH_LONG);
                t.show();
                finish();
                
            } else {
                Toast t = Toast.makeText(ErrorsWhileCopyingHandlerActivity.this,
                        getString(es.juntadeandalucia.nube.R.string.foreign_files_fail), Toast.LENGTH_LONG);
                t.show();
            }
        }
    }    
