...
package es.juntadeandalucia.nube.ui.activity;
...
import es.juntadeandalucia.nube.datamodel.FileDataStorageManager;
import es.juntadeandalucia.nube.datamodel.OCFile;
import es.juntadeandalucia.nube.files.services.FileDownloader;
import es.juntadeandalucia.nube.files.services.FileUploader;
import es.juntadeandalucia.nube.files.services.FileUploader.FileUploaderBinder;
...
import es.juntadeandalucia.nube.operations.RefreshFolderOperation;
...
import es.juntadeandalucia.nube.services.observer.FileObserverService;
import es.juntadeandalucia.nube.syncadapter.FileSyncAdapter;
import es.juntadeandalucia.nube.ui.fragment.FileDetailFragment;
import es.juntadeandalucia.nube.ui.fragment.FileFragment;
import es.juntadeandalucia.nube.ui.fragment.OCFileListFragment;
import es.juntadeandalucia.nube.ui.fragment.TaskRetainerFragment;
import es.juntadeandalucia.nube.ui.helpers.UriUploader;
import es.juntadeandalucia.nube.ui.preview.PreviewImageActivity;
import es.juntadeandalucia.nube.ui.preview.PreviewImageFragment;
import es.juntadeandalucia.nube.ui.preview.PreviewMediaFragment;
import es.juntadeandalucia.nube.ui.preview.PreviewTextFragment;
import es.juntadeandalucia.nube.ui.preview.PreviewVideoActivity;
import es.juntadeandalucia.nube.utils.DisplayUtils;
import es.juntadeandalucia.nube.utils.ErrorMessageAdapter;
import es.juntadeandalucia.nube.utils.FileStorageUtils;
import es.juntadeandalucia.nube.utils.PermissionUtil;
...
public class FileDisplayActivity extends HookActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(es.juntadeandalucia.nube.R.layout.files);

        // Navigation Drawer
        initDrawer();

        mProgressBar = (ProgressBar) findViewById(es.juntadeandalucia.nube.R.id.progressBar);
        mProgressBar.setIndeterminateDrawable(
                ContextCompat.getDrawable(this,
                        es.juntadeandalucia.nube.R.drawable.actionbar_progress_indeterminate_horizontal));

        mDualPane = getResources().getBoolean(es.juntadeandalucia.nube.R.bool.large_land_layout);
        mLeftFragmentContainer = findViewById(es.juntadeandalucia.nube.R.id.left_fragment_container);
        mRightFragmentContainer = findViewById(es.juntadeandalucia.nube.R.id.right_fragment_container);
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        if (PermissionUtil.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            ...
            if (PermissionUtil.shouldShowRequestPermissionRationale(this,
                ...
                Snackbar snackbar = Snackbar.make(findViewById(es.juntadeandalucia.nube.R.id.ListLayout), es.juntadeandalucia.nube.R.string.permission_storage_access,
                        Snackbar.LENGTH_INDEFINITE)
                        .setAction(es.juntadeandalucia.nube.R.string.common_ok, new View.OnClickListener() {
                            @Override
    ...
    protected void onAccountSet(boolean stateWasRecovered) {
        ...
        if (getAccount() != null) {
            ...
            if (mAccountWasSet) {
                setUsernameInDrawer(findViewById(es.juntadeandalucia.nube.R.id.left_drawer), getAccount());
            }
    ...
    private void createMinFragments() {
        ...
        transaction.add(es.juntadeandalucia.nube.R.id.left_fragment_container, listOfFiles, TAG_LIST_OF_FILES);
        transaction.commit();
    }
    ...
    private void setSecondFragment(Fragment fragment) {
        FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
        transaction.replace(es.juntadeandalucia.nube.R.id.right_fragment_container, fragment, TAG_SECOND_FRAGMENT);
        transaction.commit();
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        menu.findItem(es.juntadeandalucia.nube.R.id.action_sort).setVisible(!drawerOpen);
        menu.findItem(es.juntadeandalucia.nube.R.id.action_sync_account).setVisible(!drawerOpen);
        menu.findItem(es.juntadeandalucia.nube.R.id.action_switch_view).setVisible(!drawerOpen);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        inflater.inflate(es.juntadeandalucia.nube.R.menu.main_menu, menu);
        menu.findItem(es.juntadeandalucia.nube.R.id.action_create_dir).setVisible(false);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case es.juntadeandalucia.nube.R.id.action_sync_account: {
                startSynchronization();
                break;
            }
            ...
            case es.juntadeandalucia.nube.R.id.action_sort: {
                ...
                builder.setTitle(es.juntadeandalucia.nube.R.string.actionbar_sort_title)
                        .setSingleChoiceItems(es.juntadeandalucia.nube.R.array.actionbar_sortby, sortOrder,
            ...
            case es.juntadeandalucia.nube.R.id.action_switch_view: {
                if (isGridView()) {
                    item.setTitle(getString(es.juntadeandalucia.nube.R.string.action_switch_grid_view));
                    item.setIcon(ContextCompat.getDrawable(getApplicationContext(),
                            es.juntadeandalucia.nube.R.drawable.ic_view_module));
                    getListOfFilesFragment().setListAsPreferred();
                } else {
                    item.setTitle(getApplicationContext().getString(es.juntadeandalucia.nube.R.string.action_switch_list_view));
                    item.setIcon(ContextCompat.getDrawable(getApplicationContext(),
                            es.juntadeandalucia.nube.R.drawable.ic_view_list));
    ...
    private void requestUploadOfFilesFromFileSystem(Intent data, int resultCode) {
        ...
        if (filePaths != null) {
            ...
            Toast t = Toast.makeText(this, getString(es.juntadeandalucia.nube.R.string.filedisplay_no_file_selected),
                    Toast.LENGTH_LONG);
            t.show();
            return;
        }
                    ...
                    if (FileSyncAdapter.EVENT_FULL_SYNC_START.equals(event)) {
                        ...
                        if (currentDir == null) {
                            ...
                            Toast.makeText(FileDisplayActivity.this,
                                    String.format(
                                            getString(es.juntadeandalucia.nube.R.string.
                        ...
                        if (RefreshFolderOperation.EVENT_SINGLE_FOLDER_CONTENTS_SYNCED.
                            ...
                            if(RemoteOperationResult.ResultCode.UNAUTHORIZED.equals(synchResult.getCode()) ||
    ...
    private void setBackgroundText() {
        ...
        if (ocFileListFragment != null) {
            int message = es.juntadeandalucia.nube.R.string.file_list_loading;
            if (!mSyncInProgress) {
                // In case file list is empty
                message = es.juntadeandalucia.nube.R.string.file_list_empty;
            }
                ...
                if (sameAccount && sameFile && detailFragmentIsShown) {
                    ...
                    if (renamedInUpload) {
                        ...
                        Toast msg = Toast.makeText(
                                context,
                                String.format(
                                        getString(es.juntadeandalucia.nube.R.string.filedetails_renamed_in_upload_msg),
                                        newName),
                                Toast.LENGTH_LONG);
                        msg.show();
                    }
