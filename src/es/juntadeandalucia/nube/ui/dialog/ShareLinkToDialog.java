...
import es.juntadeandalucia.nube.ui.activity.CopyToClipboardActivity;
...
public class ShareLinkToDialog  extends DialogFragment {
    ...
    private AlertDialog createSelector(final boolean sendAction) {
        ...
        int titleId;
        if (sendAction) {
            titleId = es.juntadeandalucia.nube.R.string.activity_chooser_send_file_title;
        } else {
            titleId = es.juntadeandalucia.nube.R.string.activity_chooser_title;
        }
    ...
    class ActivityAdapter extends ArrayAdapter<ResolveInfo> {
        ...
        ActivityAdapter(Context context, PackageManager pm, List<ResolveInfo> apps) {
            super(context, es.juntadeandalucia.nube.R.layout.activity_row, apps);
        ...
        private View newView(ViewGroup parent) {
            return(((LayoutInflater)getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE)).
                    inflate(es.juntadeandalucia.nube.R.layout.activity_row, parent, false));
        }
        
        private void bindView(int position, View row) {
            TextView label = (TextView) row.findViewById(es.juntadeandalucia.nube.R.id.title);
            label.setText(getItem(position).loadLabel(mPackageManager));
            ImageView icon = (ImageView) row.findViewById(es.juntadeandalucia.nube.R.id.icon);
