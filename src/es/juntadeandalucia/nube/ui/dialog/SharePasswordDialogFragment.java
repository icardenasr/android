...
public class SharePasswordDialogFragment extends DialogFragment
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        View v = inflater.inflate(es.juntadeandalucia.nube.R.layout.password_dialog, null);

        // Setup layout
        EditText inputText = ((EditText)v.findViewById(es.juntadeandalucia.nube.R.id.share_password));
        inputText.setText("");
        inputText.requestFocus();

        // Build the dialog
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity(),
                es.juntadeandalucia.nube.R.style.Theme_ownCloud_Dialog_NoButtonBarStyle);
        builder.setView(v)
               ...
               .setTitle(es.juntadeandalucia.nube.R.string.share_link_password_title);
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == AlertDialog.BUTTON_POSITIVE) {
            String password =
                    ((TextView)(getDialog().findViewById(es.juntadeandalucia.nube.R.id.share_password)))
                        .getText().toString();

            if (password.length() <= 0) {
                Toast.makeText(
                        getActivity(),
                        es.juntadeandalucia.nube.R.string.share_link_empty_password,
