...
public class CreateFolderDialogFragment
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        View v = inflater.inflate(es.juntadeandalucia.nube.R.layout.edit_box_dialog, null);
        
        // Setup layout 
        EditText inputText = ((EditText)v.findViewById(es.juntadeandalucia.nube.R.id.user_input));
        inputText.setText("");
        ...
        builder.setView(v)
               .setPositiveButton(es.juntadeandalucia.nube.R.string.common_ok, this)
               .setNegativeButton(es.juntadeandalucia.nube.R.string.common_cancel, this)
               .setTitle(es.juntadeandalucia.nube.R.string.uploader_info_dirname);
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == AlertDialog.BUTTON_POSITIVE) {
            String newFolderName = 
                    ((TextView)(getDialog().findViewById(es.juntadeandalucia.nube.R.id.user_input)))
                        .getText().toString().trim();
            
            if (newFolderName.length() <= 0) {
                Toast.makeText(
                        getActivity(),
                        es.juntadeandalucia.nube.R.string.filename_empty,
            ...
            if (!FileUtils.isValidName(newFolderName, serverWithForbiddenChars)) {
                ...
                if (serverWithForbiddenChars) {
                    messageId = es.juntadeandalucia.nube.R.string.filename_forbidden_charaters_from_server;
                } else {
                    messageId = es.juntadeandalucia.nube.R.string.filename_forbidden_characters;
                }
