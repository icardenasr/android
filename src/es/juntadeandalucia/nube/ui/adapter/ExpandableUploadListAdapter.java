 ...
 */
package es.juntadeandalucia.nube.ui.adapter;
...
import es.juntadeandalucia.nube.datamodel.ThumbnailsCacheManager;
import es.juntadeandalucia.nube.datamodel.UploadsStorageManager;
import es.juntadeandalucia.nube.db.OCUpload;
import es.juntadeandalucia.nube.db.UploadResult;
import es.juntadeandalucia.nube.files.services.FileUploader;
import es.juntadeandalucia.nube.lib.common.network.OnDatatransferProgressListener;
...
import es.juntadeandalucia.nube.utils.MimetypeIconUtil;
...
public class ExpandableUploadListAdapter extends BaseExpandableListAdapter implements Observer {
    ...
    abstract class UploadGroup implements Refresh {
        ...
        public Comparator<OCUpload> comparator = new Comparator<OCUpload>() {
            ...
            public int compare(OCUpload upload1, OCUpload upload2) {
                if (upload1.getUploadStatus().equals(UploadsStorageManager.UploadStatus.UPLOAD_IN_PROGRESS)) {
                    if (!upload2.getUploadStatus().equals(UploadsStorageManager.UploadStatus.UPLOAD_IN_PROGRESS)) {
                        return -1;
                    }
    ...
    public ExpandableUploadListAdapter(FileActivity parentActivity) {
        ...
        mUploadGroups[0] = new UploadGroup(mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploads_view_group_current_uploads)) {
            @Override
            public void refresh() {
            ...
            public int getGroupIcon() {
                return es.juntadeandalucia.nube.R.drawable.upload_in_progress;
            }
        };
        mUploadGroups[1] = new UploadGroup(mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploads_view_group_failed_uploads)) {
            @Override
            public void refresh() {
            ...
            @Override
            public int getGroupIcon() {
                return es.juntadeandalucia.nube.R.drawable.upload_failed;
            }

        };
        mUploadGroups[2] = new UploadGroup(mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploads_view_group_finished_uploads)) {
            @Override
            public void refresh() {
            ...
            @Override
            public int getGroupIcon() {
                return es.juntadeandalucia.nube.R.drawable.upload_finished;
            }
    ...
    private View getView(OCUpload[] uploadsItems, int position, View convertView, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflator.inflate(es.juntadeandalucia.nube.R.layout.upload_list_item, parent, false);
        }
        ...
        if (uploadsItems != null && uploadsItems.length > position) {
            ...
            // local file name
            TextView fileTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_name);
            ...
            TextView pathTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_remote_path);
            String remoteParentPath = upload.getRemotePath();
            remoteParentPath = new File(remoteParentPath).getParent();
            pathTextView.setText(mParentActivity.getString(es.juntadeandalucia.nube.R.string.app_name) + remoteParentPath);

            // file size
            TextView fileSizeTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_file_size);
            fileSizeTextView.setText(DisplayUtils.bytesToHumanReadable(upload.getFileSize()) + ", ");

            //* upload date
            TextView uploadDateTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_date);
            ...
            TextView accountNameTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_account);
            accountNameTextView.setText(upload.getAccountName());

            TextView statusTextView = (TextView) view.findViewById(es.juntadeandalucia.nube.R.id.upload_status);

            ProgressBar progressBar = (ProgressBar) view.findViewById(es.juntadeandalucia.nube.R.id.upload_progress_bar);
            ...
            ImageButton rightButton = (ImageButton) view.findViewById(es.juntadeandalucia.nube.R.id.upload_right_button);
            if (upload.getUploadStatus() == UploadsStorageManager.UploadStatus.UPLOAD_IN_PROGRESS) {
                //Cancel
                rightButton.setImageResource(es.juntadeandalucia.nube.R.drawable.ic_cancel);
                ...
                //Delete
                rightButton.setImageResource(es.juntadeandalucia.nube.R.drawable.ic_action_delete);
                rightButton.setVisibility(View.VISIBLE);
                rightButton.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                    ...
                    view.setOnClickListener(new OnClickListener() {
                        ...
                        if (file.exists()) {
                            ...
                            final String message = String.format(
                                mParentActivity.getString(es.juntadeandalucia.nube.R.string.local_file_not_found_toast)
                            );
            ...
            ImageView fileIcon = (ImageView) view.findViewById(es.juntadeandalucia.nube.R.id.thumbnail);
            fileIcon.setImageResource(es.juntadeandalucia.nube.R.drawable.file);
            ...
            if ((fakeFileToCheatThumbnailsCacheManagerInterface.isImage()
                ...
                if ("image/png".equals(upload.getMimeType())) {
                    fileIcon.setBackgroundColor(mParentActivity.getResources()
                            .getColor(es.juntadeandalucia.nube.R.color.background_color));
                }
    ...
    private String getStatusText(OCUpload upload) {
        ...
        switch (upload.getUploadStatus()) {
            ...
            case UPLOAD_IN_PROGRESS:
                status = mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploads_view_later_waiting_to_upload);
                FileUploader.FileUploaderBinder binder = mParentActivity.getFileUploaderBinder();
                if (binder != null && binder.isUploadingNow(upload)) {
                    /// really uploading, bind the progress bar to listen for progress updates
                    status = mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploader_upload_in_progress_ticker);
                }
                break;

            case UPLOAD_SUCCEEDED:
                status = mParentActivity.getString(es.juntadeandalucia.nube.R.string.uploads_view_upload_status_succeeded);
                break;
                ...
                switch (upload.getLastResult()) {
                    case CREDENTIAL_ERROR:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_credentials_error
                        );
                        break;
                    case FOLDER_ERROR:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_folder_error
                        );
                        break;
                    case FILE_NOT_FOUND:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_localfile_error
                        );
                        break;
                    case FILE_ERROR:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_file_error
                        );
                        break;
                    case PRIVILEDGES_ERROR:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_permission_error
                        );
                        break;
                    case NETWORK_CONNECTION:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_failed_connection_error
                        );
                        break;
                    case DELAYED_FOR_WIFI:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_waiting_for_wifi
                        );
                        break;
                    case CONFLICT_ERROR:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_conflict
                        );
                        break;
                    case SERVICE_INTERRUPTED:
                        status =  mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_service_interrupted
                        );
                        break;
                    case UNKNOWN:
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_unknown_fail
                        );
                        break;
                    case CANCELLED:
                        // should not get here ; cancelled uploads should be wiped out
                        status = mParentActivity.getString(
                            es.juntadeandalucia.nube.R.string.uploads_view_upload_status_cancelled
                        );
                        break;
                    case UPLOADED:
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = inflaInflater.inflate(es.juntadeandalucia.nube.R.layout.upload_list_group, null);
        }
        TextView tv = (TextView) convertView.findViewById(es.juntadeandalucia.nube.R.id.uploadListGroupName);
